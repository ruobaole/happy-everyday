...
//--- r2 ---//
//
// First, get the sorting order of the array by comparing the numbers in the
//  2 ends -- isAsc = arr[0] <= arr[end]
// Second, binary search within the array; the searching order is affected by
//  isAsc
//
// Time: O(logN)
// Space: O(1)

export function binarySearch_r2(arr: number[], key: number): number {
  if (arr.length === 0) {
    return -1
  }
